...
void mmc_read_block(uint32_t block_address, uint8_t *data)
{
    regmask32(MMC0 + SD_BLK, SD_BLK_BLEN, SD_BLK_BLEN_512);
    if (!mmc_send_command((17 << SD_CMD_INDX) | SD_CMD_CICE | SD_CMD_CCCE | SD_CMD_RSP_TYPE_48 | SD_CMD_DP_YES | SD_CMD_DDIR_READ, block_address))
        while (1);
    while (!(regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BRE));
    uint32_t *data_cursor = (uint32_t *)data;
    while (regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BRE)
        *data_cursor++ = regread32(MMC0 + SD_DATA);
}
