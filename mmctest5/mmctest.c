...
void mmctest(void)
{
    ...
    console_puts("insert card and press a key\r\n");
    console_getc();
    ...
    console_hexprint(long_response[3]);
    console_hexprint(long_response[2]);
    console_hexprint(long_response[1]);
    console_hexprint(long_response[0]);
    console_puts("\r\n");
    ...
    while (regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BRE)
        console_hexprint(regread32(MMC0 + SD_DATA));
    console_puts("\r\n");

    console_puts("cmd24\r\n");
    regmask32(MMC0 + SD_BLK, SD_BLK_BLEN, SD_BLK_BLEN_512);
    if (!mmc_send_command((24 << SD_CMD_INDX) | SD_CMD_CICE | SD_CMD_CCCE | SD_CMD_RSP_TYPE_48 | SD_CMD_DP_YES | SD_CMD_DDIR_WRITE, 1))
        while (1);
    while (!(regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BWE));
    while (regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BWE)
    {
        regwrite32(MMC0 + SD_DATA, 0xcafeb00b);
        console_puts(".");
    }
    console_puts("\r\n");
    while (!(regread32(MMC0 + SD_STAT) & SD_STAT_TC));
    regmask32(MMC0 + SD_STAT, SD_STAT_TC, SD_STAT_TC);

    console_puts("cmd17\r\n");
    regmask32(MMC0 + SD_BLK, SD_BLK_BLEN, SD_BLK_BLEN_512);
    if (!mmc_send_command((17 << SD_CMD_INDX) | SD_CMD_CICE | SD_CMD_CCCE | SD_CMD_RSP_TYPE_48 | SD_CMD_DP_YES | SD_CMD_DDIR_READ, 1))
        while (1);
    while (!(regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BRE));
    while (regread32(MMC0 + SD_PSTATE) & SD_PSTATE_BRE)
        console_hexprint(regread32(MMC0 + SD_DATA));
